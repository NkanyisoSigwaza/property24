...
router.post("/getProperty",function(req,res){
  ...
  Property.find({agent:req.body.agent}).then(function(property){    // returns updated ninja
    res.send(property);
  })
...
router.post("/addProperty",function(req,res,next){
  //res.send(req.body);
   ...
   Property.img.data= fs.readFileSync(req.files.userPhoto.path); //array of numbers representing our img
   Property.name.contentType = ‘image/png’;
   Property.save();
